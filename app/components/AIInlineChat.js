    ...
    <div className="flex flex-col h-full bg-gradient-to-b from-[#1a1a1d] to-[#2a2a2d] border border-gray-700 rounded-2xl shadow-xl overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-[#2a2a2d] to-[#3a3a3d] p-3 flex items-center justify-between border-b border-gray-700">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-gradient-to-br from-gray-600 to-gray-700 rounded-lg flex items-center justify-center shadow-sm text-white">🤖</div>
          <div>
            <div className="text-sm font-semibold text-white">Biscript AI</div>
            <div className="text-xs text-gray-400">Ask about lessons or coding</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={clearChat} className="text-xs px-2 py-1 cursor-pointer bg-gray-700 hover:bg-gray-600 rounded text-white">Clear</button>
          <button onClick={onClose} className="text-xs px-2 py-1 cursor-pointer bg-transparent hover:bg-gray-700 rounded text-white">Close</button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto p-3 hide-scrollbar">
        {messages.length === 0 && (
          <div className="text-center text-gray-400 text-sm py-6">Chat with Biscript — quick, concise help for your code.</div>
        )}
        ...
        <div className="flex flex-col gap-3">
          {messages.map((m) => (
            <div
              key={m.id}
              className={`w-full flex items-start mb-3 ${m.role === "user" ? "justify-end" : "justify-start"
                } min-w-0`}
            >
              {/* AI Avatar */}
              {m.role === "ai" && (
      ...
      <form onSubmit={sendMessage} className="p-3 border-t border-gray-700 bg-gradient-to-r from-[#2a2a2d] to-[#232526] flex gap-2">
        <input
          value={input}
        ...
        />
        <button type="submit" disabled={loading} className="h-10 px-3 bg-[#222222] cursor-pointer text-[#28c244] rounded hover:bg-gray-700 transition">{loading ? '...' : 'Send'}</button>
      </form>
    </div>
  );
}
