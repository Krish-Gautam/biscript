...
import { getGoblinLineByLesson } from "../services/goblinLineServices";
...
export default forwardRef(function CodeEditor({ initialLanguage, initialLesson, onLanguageChanged }, ref) {
  ...
  useEffect(() => {
    ...
    const fetchLessons = async () => {
      ...
      try {
        const data = await getLessonByLanguage(initialLanguage);
  ...
  useEffect(() => {
    ...
    const handleLessonInitialization = async (lesson) => {
      ...
      if (!lessonQuestions[lesson.id]) {
        const questions = await getQuestionByLesson(lesson.id);
  ...
  useEffect(() => {
    ...
    const fetchGoblinTeaching = async () => {
      const { data } = await getGoblinLineByLesson(currentLesson.id);
      if (data?.length > 0) {
  ...
  const handleLessonClick = async (lesson) => {
    ...
    navigate(`/questions/${initialLanguage}/${lesson.id}`);
  };
  ...
  useImperativeHandle(ref, () => ({
    ...
    setLanguage: async (name) => {
      ...
      const fetchedLessons = await getLessonByLanguage(name.toLowerCase());
      if (matched) {
        navigate(`/questions/${matched.name.toLowerCase()}/${fetchedLessons[0]?.id}`);
      }
    },
